...
# Inicializar variables de sesión siempre
...
if not st.session_state.logged_in:
    ...
    # Obtener último cierre para filtrar ventas
    try:
        conn = get_connection()
        cur = conn.cursor()
    ...
    try:
        ...
        df_prod_hoy = pd.read_sql(
            f"SELECT COALESCE(SUM(cantidad), 0) as total FROM movimientos WHERE tipo='produccion' AND fecha > {desde}", engine)
        c4.metric("👩‍🍳 Producido", f"{df_prod_hoy['total'].iloc[0]:.1f}")
        ...
        with col_g1:
            ...
            if not df_v.empty:
                ...
                st.info("Sin ventas registradas en el período actual.")
