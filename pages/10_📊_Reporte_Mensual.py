...
if st.button("🔍 Generar Reporte", type="primary", use_container_width=True):
    ...
    total_ventas = float(df_ventas['total'].sum()) if not df_ventas.empty else 0.0
    total_compras = float(df_compras['gasto'].sum()) if not df_compras.empty else 0.0
    ...
    c4.metric("🧾 Tickets", int(df_ventas['tickets'].sum()) if not df_ventas.empty else 0)
    ...
    if not df_productos.empty:
        ...
        for _, row in df_productos.head(10).iterrows():
            data_p.append([
                Paragraph(str(row['nombre']), ParagraphStyle('cell', fontSize=8, wordWrap='CJK')),
                f"{row['unidades']:.1f}",
                f"${row['facturado']:,.2f}"
            ])
        tp = Table(data_p, colWidths=[220, 70, 90])
    ...
    if not df_compras.empty:
        elements.append(Paragraph("Compras del Mes", h2))
        data_c = [["Producto", "Cantidad", "Gasto"]]
        ...
        tc = Table(data_c, colWidths=[220, 70, 90])
